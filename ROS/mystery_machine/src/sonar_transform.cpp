...
void transformPoint(const tf::TransformListener& listener) {
    ...
    try {
        ...
        ROS_INFO("base_sonar: (%.2f, %.2f. %.2f) -----> "
            ...
            sonar_point.point.x, sonar_point.point.y,
            sonar_point.point.z, base_point.point.x,
        ...
        ROS_ERROR("Received an exception when transforming "
            "from \"base_sonar\" to \"base_link\": %s", ex.what());
    }
}
...
int main(int argc, char** argv) {
    ...
    ros::Timer timer = n.createTimer(ros::Duration(0.5),
        boost::bind(&transformPoint, boost::ref(listener)));

    pub = new ros::Publisher(n.advertise<geometry_msgs::PointStamped>(
        "sonar_transformed", 50));
